...
## Importing data
movie_budgets <- read_csv("movie_budgets.txt")
...
```{r}
ggplot(data=movie_budgets, aes(x=year, y=log10budget)) +
  geom_jitter(alpha=.2) +
  facet_wrap(~ cut_number(length, 10), ncol=5) +
  stat_smooth(method="loess", formula="y~x")

ggplot(data=movie_budgets, aes(x=length, y=log10budget)) +
  geom_jitter(alpha=.2) +
  facet_wrap(~ cut_number(year, 10), ncol=5) +
  stat_smooth(method="loess", formula="y~x")
```
...
```{r}
ggplot(data=augment(loess_model), aes(x=.fitted, y=.resid)) +
  geom_point() +
  stat_smooth(formula="y~x", method="loess", se=F)

ggplot(data=augment(loess_model), aes(x=.fitted, y=abs(.resid))) +
  geom_point() +
  stat_smooth(formula="y~x", method="loess", se=F)

ggplot(data=augment(loess_model)) +
